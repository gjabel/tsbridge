...
for (p in 0:8) {
  ...
  # write the BUGS model to .txt file in the working directory for use in the bugs function of R2OpenBUGS
  ...
  #parameter names
  pp <- nodes(cv, "prior")$name
  # save mcmc results in appropriate directory
  param.mcmc <- as.mcmc(cv.bug$sims.matrix[, pp])
...
##
## Calculate CV model density, q1(w_i), multivariate normal density, q2(w_i), and their ratio, l(w_i), for both the bugs MCMC (w_1) and a simulated 
## multivariate normal sample (w_2)
##
library("tsbridge")
